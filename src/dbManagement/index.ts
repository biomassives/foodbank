...
export async function syncEntryToCloud(entry: Entry, orgId: string | null = null) {
  ...
  // Upload sketch or photo to Storage when present (base64 data URL → blob)
  let mediaUrl: string | null = null;
  const mediaData = entry.sketch || entry.image;
  if (mediaData?.startsWith('data:')) {
    try {
      const blob = await (await fetch(mediaData)).blob();
      const ext = entry.sketch ? 'png' : 'jpg';
      const path = `${user.id}/${entry.id}.${ext}`;

  ...
  <q-page class="index-page">
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 18]">
      <div class="fab-group">
        ...
        <q-btn fab-mini icon="add_location" class="fab-btn" @click="fabOpen = true" />
      </div>
    </q-page-sticky>

    <entry-modal v-model:card-state="fabOpen" :edit-entry="editEntryTarget" :initial-type="entryInitialType" @saved="handleEntrySaved" />
...
// Filter support — driven by ?filter=<type> query param
const activeFilter = computed(() => route.query.filter as string || null);
const isCommunityFilter = computed(() =>
  ['need', 'offering', 'looking_for', 'upcoming_need'].includes(activeFilter.value || '')
);

function clearFilter() { router.push({ path: '/' }); }

function filterLabel(f: string) {
  const map: Record<string, string> = {
    contacts: 'CONTACTS', need: 'NEEDS', offering: 'OFFERINGS',
    looking_for: 'LOOKING FOR', upcoming_need: 'UPCOMING',
  };
  return map[f] || f.toUpperCase().replace(/_/g, ' ');
}

function filterIcon(f: string) {
  const map: Record<string, string> = {
    contacts: 'people', need: 'volunteer_activism', offering: 'card_giftcard',
    looking_for: 'search', upcoming_need: 'event',
  };
  return map[f] || 'filter_list';
}

function filterColor(f: string) {
  const map: Record<string, string> = {
    contacts: 'var(--wb-info)', need: 'var(--wb-negative)', offering: 'var(--wb-positive)',
    looking_for: 'var(--wb-queue-transit)', upcoming_need: 'var(--wb-warning)',
  };
  return map[f] || 'var(--wb-text-faint)';
}
...
const entryInitialType = ref<string | null>(null);
...
function filterAdd() {
  if (activeFilter.value === 'contacts') {
    ...
    entryInitialType.value = activeFilter.value;
    editEntryTarget.value = null;
    fabOpen.value = true;
  }
}
...
/* Shared base — both buttons: same size, same dark bg, colored border */
.fab-loc-btn,
.fab-btn {
...
.fab-btn {
  ...
  box-shadow: 0 2px 12px var(--wb-fab-glow);
}
.fab-btn:hover {
